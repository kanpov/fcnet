...
async fn add_without_netns(cli: Cli, netlink_handle: rtnetlink::Handle) {
    ...
    run_iptables(
        &cli,
        format!("-A FORWARD -i {} -o {} -j ACCEPT", cli.tap_name, cli.iface_name),
    )
    .await;
}
...
async fn del_without_netns(cli: Cli, netlink_handle: rtnetlink::Handle) {
    ...
    run_iptables(
        &cli,
        format!("-D FORWARD -i {} -o {} -j ACCEPT", cli.tap_name, cli.iface_name),
    )
    .await;
}
...
async fn check_without_netns(cli: Cli, netlink_handle: rtnetlink::Handle) {
    ...
    run_iptables(
        &cli,
        format!("-C FORWARD -i {} -o {} -j ACCEPT", cli.tap_name, cli.iface_name),
    )
    .await;
}
